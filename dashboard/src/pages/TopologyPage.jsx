...
// Bidirectional marker for edges that go both ways
...
const staticEdges = [
    ...
    { id: 'e-edge-discovery', source: 'EDGE', target: 'DISCOVERY', sourceHandle: 'right-source', targetHandle: 'right-target', data: { protocol: 'UDP' }, ...bidirectionalMarkers },
    { id: 'e-dc-discovery', source: 'DATACENTER', target: 'DISCOVERY', sourceHandle: 'right-source', targetHandle: 'right-target', data: { protocol: 'UDP' }, ...bidirectionalMarkers },
    { id: 'e-auth-discovery', source: 'AUTH', target: 'DISCOVERY', sourceHandle: 'right-source', targetHandle: 'right-target', data: { protocol: 'UDP' }, ...bidirectionalMarkers },
];
...
function TopologyContent() {
    ...
    // Get everything from TraceContext (including filters now)
    const {
        events,
        allEvents,
        filteredAllEvents,
    ...
    // Use filteredAllEvents for playback (so animation matches filtered list)
    ...
    // Auto-navigate sidebar to show the currently animating event
    useEffect(() => {
    ...
    // Derive external clients from allEvents (not filtered - we want all nodes always)
    ...
    // Build client to address map for displaying addresses on nodes
    // Uses both remoteAddress (from server perspective) and localAddress (from client perspective)
    const clientToAddressMap = useMemo(() => {
        ...
        for (const event of allEvents) {
            ...
            // Case 2: Client's own event with localAddress - also useful
            // This captures the client's ephemeral port when it sends messages
    ...
    // Compute dynamic addresses for nodes based on current event
    const eventAddresses = useMemo(() => {
        ...
        // The peer's address is remoteAddress from the component's perspective
        if (peer && remoteAddr) {
            if (isExternalClient(peer)) {
                addresses.set(peer, remoteAddr);
            }
            // If peer was an IP address that resolved to a client
        ...
        // If this is a client event, show its localAddress (the ephemeral port it's using)
        if (isExternalClient(currentEvent.componentType) && localAddr) {
            addresses.set(component, localAddr);
        }
        
        // If peerId is an unresolved IP address, also set it for display
        // This handles the case where PacketFilter receives from unknown IP
        if (isIpAddress(currentEvent.peerId) && remoteAddr) {
            const normalizedPeerId = normalizeAddress(currentEvent.peerId);
            // Only if we couldn't resolve it to a known client
    ...
    // Event clicked in sidebar - uses seekTo to sync with timeline
