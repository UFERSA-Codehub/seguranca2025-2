...
public class TcpHandler implements Runnable {
    private static final Logger logger = LoggerFactory.getLogger("Auth.TcpHandler");
    ...
    private boolean performHandshake() {
        ...
        logger.debug("CHALLENGE enviado para {}", peerId);
    ...
    private void handleMessages() {
        while (!clientSocket.isClosed()) {
            ...
            if (message == null) {
                logger.debug("Conexão fechada por {}", peerId);
                break;
            }

            if (!channel.isEncryptedEnvelope(message)) {
                logger.warn("Mensagem não cifrada de {} - ignorando", peerId);
                continue;
            }

            if (!channel.verify(message)) {
                logger.warn("Falha na verificação de mensagem de {}", peerId);
                continue;
            }
            ...
            switch (envelope.getType()) {
                ...
                default -> logger.warn("Tipo de mensagem não suportado: {}", envelope.getType());
            }
        }
    }
    ...
    private void handleAuth(EnvelopeTCP envelope) {
        ...
        logger.info("AUTH de sensor {}", sensorId);
        ...
        if (credentials.validateSensor(sensorId, password)) {
            ...
            logger.info("AUTH_OK enviado para {}", sensorId);
        } else {
            MessageTCP responseMsg = channel.buildEncryptedEnvelope(
                peerId, MessageTypeTCP.AUTH_FAIL, "Credenciais inválidas"
            );
            channel.send(responseMsg);
            logger.warn("AUTH_FAIL para {} - credenciais inválidas", sensorId);
        }
    }
    ...
    private void handleValidate(EnvelopeTCP envelope) {
        ...
        logger.info("VALIDATE de usuário {}", username);
        ...
        if (credentials.validateUser(username, password)) {
            ...
            MessageTCP responseMsg = channel.buildEncryptedEnvelope(
                peerId, MessageTypeTCP.VALIDATE_FAIL, "Credenciais inválidas"
            );
            channel.send(responseMsg);
            logger.warn("VALIDATE_FAIL para {} - credenciais inválidas", username);
        }
    }
