...
public class SensorTcpHandler implements Runnable {
    ...
    public void forceClose() {
        ...
        try {
            ...
            logger.debug("Erro ao fechar socket forçadamente: {}", e.getMessage());
        }
    }
    ...
    private void handleData(EnvelopeTCP envelope) {
        ...
        if (token == null || !jwt.isValid(token)) {
            logger.warn("Token JWT inválido ou ausente de '{}'", peerId);
            sendError("Token JWT inválido");
            return;
        }
    ...
    private void cleanup() {
        ...
        if (forceTerminated) {
            logger.warn("Conexão de '{}' (IP: {}) terminada pelo IDS", peerId, clientIp);
        }
        if (channel != null) {
            if (peerId != null) {
