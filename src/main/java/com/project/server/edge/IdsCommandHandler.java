...
public class IdsCommandHandler implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            logger.info("Handshake com IDS concluido ({})", peerId);

            while (!idsSocket.isClosed()) {
                MessageTCP message = channel.receive();
                if (message == null) {
                    logger.debug("Conexao IDS fechada");
                    break;
                }

                if (!channel.verify(message)) {
                    logger.warn("Mensagem invalida do IDS");
                    continue;
                }
            ...
            logger.error("Erro na conexao com IDS: {}", e.getMessage());
        } finally {
            try {
                idsSocket.close();
            } catch (IOException e) {
    ...
    private void handleMessage(SecureTCPChannel channel, String peerId, MessageTCP message) {
        if (channel.isEncryptedEnvelope(message)) {
            ...
            if (envelope.getType() == MessageTypeTCP.TERMINATE) {
                ...
                logger.warn("Tipo de mensagem nao suportado do IDS: {}", envelope.getType());
            }
